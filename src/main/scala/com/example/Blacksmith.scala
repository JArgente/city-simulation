...
class Blacksmith(context: ActorContext[City.CityInfo], actorInfo: ActorInfo)
  ...
  private def getNextDay(cityInfo: City.CityInfo, actorInfo: ActorInfo): BlacksmithAction = {
      if(actorInfo.status.wealth<3) {
        BlacksmithAction(0,1,this.context.self)
      } else if(actorInfo.personality.entrepeneur > 5) {
        if(actorInfo.personality.greed >7)
          BlacksmithAction(1,1,this.context.self)
        else
          BlacksmithAction(1,0,this.context.self)
      } else
        BlacksmithAction(0,0,this.context.self)
  }
